...
impl Resources {
    pub fn load() -> eyre::Result<Resources> {
        ...
        for path in all_subpaths(&tex_root, "png")? {
            ...
            let tex = mq::Texture2D::from_file_with_format(
                &file,
...
static THE_ASSETS: Mutex<Option<Resources>> = Mutex::new(None);

pub struct AssetsRef(MutexGuard<'static, Option<Resources>>);

impl Deref for AssetsRef {
